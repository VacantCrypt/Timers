...
class TimerApp:
    def __init__(self, master):
        ...
        self.minutes_label = tk.Label(self.custom_frame, text="Mins:")
        self.minutes_label.grid(row=0, column=0, sticky=tk.W)

        self.minutes = tk.Entry(self.custom_frame, width=5)
        self.minutes.grid(row=0, column=1)
        self.minutes.insert(tk.END, "0")  # Set default text in minutes Entry widget

        self.seconds_label = tk.Label(self.custom_frame, text="Secs:")
        self.seconds_label.grid(row=0, column=2, sticky=tk.W)

        self.seconds = tk.Entry(self.custom_frame, width=5)
        self.seconds.grid(row=0, column=3)
        self.seconds.insert(tk.END, "0")  # Set default text in seconds Entry widget

        self.add_button = tk.Button(self.custom_frame, text="Add", command=self.add_custom_timer)
        self.add_button.grid(row=0, column=4)

        # Bind shortcut key for the "Add" button
        if self.add_shortcut:
            keyboard.add_hotkey(self.add_shortcut, self.add_custom_timer)
    ...
    def load_config(self):
        ...
        self.add_shortcut = None  # Default shortcut for the "Add" button
        self.timer_4_hotkey = None  # Hotkey for timer 4
        self.timer_5_hotkey = None  # Hotkey for timer 5
        self.timer_4_time = 120  # Default time for timer 4 (2 minutes)
        self.timer_5_time = 60  # Default time for timer 5 (1 minute)
        ...
        with open(self.config_file_path, 'r') as config_file:
            ...
            for line in lines:
                if line.startswith("Sound File Path"):
                ...
                elif line.startswith("Hot Key for timer 4"):
                    self.timer_4_hotkey = line.split(':')[1].strip()
                elif line.startswith("Hot Key for timer 5"):
                    self.timer_5_hotkey = line.split(':')[1].strip()
                elif line.startswith("Timer 4"):
                    self.timer_4_time = int(line.split(':')[1].strip())
                elif line.startswith("Timer 5"):
                    self.timer_5_time = int(line.split(':')[1].strip())
                    ...
                    if hotkey:  # Check if hotkey is specified
                ...
                elif line.startswith("Hot Key for Add"):
                    self.add_shortcut = line.split(':')[1].strip()
        ...
        # Bind hotkeys for timer 4 and 5 if specified
        if self.timer_4_hotkey:
            keyboard.add_hotkey(self.timer_4_hotkey, lambda: self.add_timer(0, self.timer_4_time))
        if self.timer_5_hotkey:
            keyboard.add_hotkey(self.timer_5_hotkey, lambda: self.add_timer(0, self.timer_5_time))
    ...
    def handle_hotkey(self, index):
        if index == 0:
            ...
            self.add_timer(0, self.timer_4_time)
        elif index == 4:
            self.add_timer(0, self.timer_5_time)
    ...
    def create_default_config(self):
        ...
        with open(self.config_file_path, 'w') as config_file:
            ...
            config_file.write("Hot Key for timer 4: ctrl+4\n")
            config_file.write("Hot Key for timer 5: ctrl+5\n")
            config_file.write("Hot Key for Sort: ctrl+6\n")
            config_file.write("Hot Key for Add: ctrl+7\n")
            config_file.write("-----------------------------\n")
            config_file.write("Timer 4: 120\n")  # Default time for timer 4 (2 minutes)
            config_file.write("Timer 5: 60\n")   # Default time for timer 5 (1 minute)
            config_file.write("-----------------------------\n")
    ...
    def add_custom_timer(self):
        minutes_text = self.minutes.get()
        seconds_text = self.seconds.get()

        # Check if both fields are non-empty
        if minutes_text and seconds_text:
            try:
                ...
                print("Invalid input. Please enter valid integer values for minutes and seconds.")
        else:
            # Handle empty fields
            print("Please enter values for both minutes and seconds.")
